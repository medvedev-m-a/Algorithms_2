...
class SimpleTree:
    ...
    def GetAllChild(self, all_nodes: list, node: SimpleTreeNode) -> list:
    ...
    def EvenTrees(self) -> list:
        delete_edge = []
        self.FindEdge(self.Root, delete_edge)
        return delete_edge

    def FindEdge(self, node: SimpleTreeNode, delete_edge: list) -> None:
        if len(node.Children) > 1:
            for ch in node.Children:
                all_nodes_in_tree = []
                if len(self.GetAllChild(all_nodes_in_tree, ch)) % 2 == 0:
                    delete_edge.append(node)
                    delete_edge.append(ch)
                self.FindEdge(ch, delete_edge)
